                      ...
                      <div class="back d-flex flex-column cont backs">
                         <div class="back-content backs-content">
                            <div class="mpp-controls"> 
                               <div class="mpp-contr-btn mpp-link-icon mpp-buttons-bars" data-tooltip="Play Now">
...
function updateProgressBar() {
  if (index11 > 0) {
   if (booleans1)  {
      let bottomProgressBar = document.getElementById(`${"bottom-progress-bar" + currentSelectedId}`) ; 
      let bottomProgressBar1 = document.getElementById(`${"bottom-progress-bar1" + currentSelectedId}`) ; 
  ...
  seasonOne.forEach((element) => { 
    if (element.id === currentSelectedId) { 
          ...
          if (bottomProgressBar1 && !booleans11) {
...
let categorysCount = [
    'actions' , 'comics' , 'catastrophic' , 'adventure'  , 'Dramatic' , 'Documentary' , 'War' , 'Horror' , 
] ; 
...
function updateProgressBar1(ind , boolean) { 
  ...
  if (topProgressBar.length ) {  
     percantages1 = Math.floor((100 / playersssss[0].duration) * playersssss[0].currentTime);
       seasonOne.forEach((element) => { 
        if (parseInt(element.id) === (index + 1)) { 
